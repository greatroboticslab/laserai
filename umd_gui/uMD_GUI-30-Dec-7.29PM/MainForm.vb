            ...
            Try
                ...
                For k = 0 To sets.Length - 1
                    ...
                    If values.Length.Equals(8) Or values.Length.Equals(16) Then
                        ...
                        ' === MQTT: publish same format as capture log ("D:<raw> N:<index>") ===
                        Try
                            Dim ax As Integer = PrimaryAxisSelect

                            ' Match the capture log mapping:


                            Dim D As Long = CLng(AxisData(ax, 2)) ' D: values(2)
                            Dim N As Long = CLng(AxisData(ax, 5)) ' N: values(5)

                            ' Exact same string as the log file
                            Dim payload As String = "D:" & D.ToString() & " N:" & N.ToString() 'format D: 12345667 N: 8901234

                            Await _bridge.PublishAsync(payload)

                        Catch ex As Exception
                            Console.WriteLine("MQTT publish error: " & ex.Message)
                        End Try
                        ' === END MQTT publish block ===
